...
class ArchimedesTag extends Archimedes {
	...
	public static function archimedes_post_meta() {
		...
		$date = sprintf( __( ' on <a href="%1$s" title="%2$s" rel="bookmark"><time datetime="%3$s">%4$s</time></a>', 'archimedes' ),
		...
		$author = sprintf( __( ' by <a href="%1$s" rel="author">%2$s</a>', 'archimedes' ),
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			get_the_author()
		);
		
		$edit = current_user_can( 'edit_post', $post->ID ) ? sprintf( __( '<a href="%s" class="post-edit-link">Edit This</a>', 'archimedes' ), get_edit_post_link() ) : '';
		
		if ( webcomic() and is_a_webcomic() ) {
			$meta = sprintf( __( 'Published%1$s%2$s%3$s%4$s%5$s<span class="post-actions">%6$s%7$s%8$s</span>', 'archimedes' ),
				WebcomicTag::get_the_webcomic_collection_list( 0, __( ' in ', 'archimedes' ) ),
				WebcomicTag::get_the_webcomic_term_list( 0, 'storyline', __( ' as part of ', 'archimedes' ) ),
				WebcomicTag::get_the_webcomic_term_list( 0, 'character', __( ' featuring ', 'archimedes' ) ),
				$date,
				$author,
				webcomic_transcripts_open() ? WebcomicTag::webcomic_transcripts_link( '%link' ) : '',
				webcomic_prints_available() ? WebcomicTag::purchase_webcomic_link( '%link', __( 'Purchase', 'archimedes' ) ) : '',
				$edit
			);
		} else if ( is_attachment() ) {
			$data = wp_get_attachment_metadata();
			$meta = sprintf( __( 'Published%1$s%2$s%3$s%4$s<span class="post-actions">%5$s</span>', 'archimedes' ),
				$post->post_parent ? sprintf( __( ' in <a href="%1$s" title="Return to %2$s" rel="gallery">%3$s</a>', 'archimedes' ),
					esc_url( get_permalink( $post->post_parent ) ),
					esc_attr( strip_tags( get_the_title( $post->post_parent ) ) ),
					get_the_title( $post->post_parent )
				) : '',
				isset( $data[ 'width' ], $data[ 'height' ] ) ? sprintf( __( ' at <a href="%1$s" title="Link to full-size image">%2$s &times; %3$s</a>', 'archimedes' ),
...
if ( !class_exists( 'Walker_ArchimedesNavMenu_Dropdown' ) ) {
	/** Handle responsive dropdown custom menu output.
	 * 
	 * @package Archimedes
	 */
	class Walker_ArchimedesNavMenu_Dropdown extends Walker_Nav_Menu {
...
if ( !class_exists( 'Walker_ArchimedesPageMenu_Dropdown' ) ) {
	/** Handle responsive dropdown page menu output.
	 * 
	 * @package Archimedes
	 */
	class Walker_ArchimedesPageMenu_Dropdown extends Walker_PageDropdown {
		/** Override parent function. */
		function start_lvl( &$output, $depth ){}
